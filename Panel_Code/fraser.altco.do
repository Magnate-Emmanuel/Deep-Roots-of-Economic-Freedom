...
vce(cluster cid) noomitted vsquish
/******************************************************/
...
/* COLUMN 1 */
/* -------- */
eststo col1: reg $ylist $xlist $geor $climate $ethalt1 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
pcorr $ylist $xlist $geor $climate $ethalt1 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if e(sample) == 1 
...
/* COLUMN 2 */
/* -------- */
eststo col2: reg $ylist $xlist $geor $climate $ethalt2 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $climate $ethalt2 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 3 */
/* -------- */
eststo col3: reg $ylist $xlist $geor $climate $ethalt3 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
*outreg2 using econfree.rtf, append keep(avg_pdiv_aa $geor) ctitle(OLS) 
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $climate $ethalt3 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 4 */
/* -------- */
eststo col4: reg $ylist $xlist $geor $climate $ethalt4 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr reg $ylist $xlist $geor $climate $ethalt4 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
 if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 5 */
/* -------- */
eststo col5: reg $ylist $xlist $geor $climate $ethbase $moregeo $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $climate $ethbase $moregeo $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
estadd scalar pc_pdiv = pc_mtrx[rownumb(pc_mtrx, "$xlist"), 1]^2 : col5 

/***********************************************************************/
/* IV
ivregress 2sls $ylist $geor $climate $ethalt1 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum ($xlist = $ivlist) if smpl_flag2 == 1, vce(cluster cid) first noomitted vsquish
ivregress 2sls $ylist $geor $climate $ethalt2 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum ($xlist = $ivlist) if smpl_flag2 == 1, vce(cluster cid) first noomitted vsquish
ivregress 2sls $ylist $geor $climate $ethalt3 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum ($xlist = $ivlist) if smpl_flag2 == 1, vce(cluster cid) first noomitted vsquish
ivregress 2sls $ylist $geor $climate $ethalt4 $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum ($xlist = $ivlist) if smpl_flag2 == 1, vce(cluster cid) first noomitted vsquish
ivregress 2sls $ylist $geor $climate $ethbase $moregeo $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum ($xlist = $ivlist) if smpl_flag2 == 1, vce(cluster cid) first noomitted vsquish */


/* -------- */
/* COLUMN 6 */
/* -------- */
eststo col6: reg $ylist $xlist $geor $climate $ethineq $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* COLUMN 7 */
/* -------- */
eststo col7: reg $ylist $xlist $geor $climate $ethineq $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* COLUMN 9 */
/* -------- */
eststo col9: reg $ylist $xlist $geor $climate $ethineq $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $climate $deepdet $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 11 */
/* --------- */
eststo col11: ivregress 2sls $ylist $geor $climate $ethineq $timedum ($xlist = $ivlist) if smpl_flag1 == 1, vce(cluster cid) first noomitted vsquish 
...
eststo col11_fs: reg $xlist $ivlist $geor $climate if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
estadd scalar fs_fsta = fs_mtrx[1,4] : col11_fs 

/* --------- */
/* COLUMN 12 */
/* --------- */
eststo col12: ivregress 2sls $ylist $geor $climate $ethineq $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall $timedum ($xlist = $ivlist) if smpl_flag2 == 1, vce(cluster cid) first noomitted vsquish
...
/* Obtain the first-stage regression coefficients and the first-stage F-statistic */
eststo col12_fs: reg $xlist $ivlist $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall if smpl_flag2 == 1, vce(cluster cid) noomitted vsquish
...
estout col1 col2 col3 col4 col5 col6 col7 col8 col9 col10 col11 col12 using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Results\Tables\mtab_freqy.tex", style(tex) replace cells(b(star fmt(a2)) se(fmt(a2) par)) drop(${contall} ${legalor} _cons, relax) order($xlist $pd_vars $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $colhist ) indicate("Continent dummies=${contall}" "Legal origin dummies=${legalor}" , labels("\$\times\$" " ")) stats(N pc_pdiv pc_pdiv_w pc_pdiv_b adjr2, fmt(%9.0f a2 a2 a2 a2) labels("Observations"  "Partial \$R^2\$ of population diversity"  "Partial \$R^2\$ of within-group"  "Partial \$R^2\$ of between-group"  "Adjusted \$R^2\$") layout(@ @ @ @ @)) varwidth(50) msign("\$-\$") label prehead("\begin{tabular*}{895pt}{@{\extracolsep{\fill}}lcccccccccccc}" "\toprule" "Cross-country sample:&\multicolumn{9}{c}{Global}&\multicolumn{1}{c}{Old World}&\multicolumn{2}{c}{Global}\\" "\cmidrule(r){2-9}\cmidrule(lr){10-11}\cmidrule(l){12-13}") numbers mlabels("OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "2SLS" "2SLS") collabels(none) posthead("\midrule"  "&\multicolumn{12}{c}{Economic Freedom per year, 1970--2017}\\"  "\cmidrule{2-13}") varlabels(, elist(${inst} \addlinespace)) prefoot("\midrule") postfoot("\addlinespace") 

estout col1_me col2_me col3_me col4_me col5_me col6_me col7_me col9_me col10_me col11_me col12_me using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Results\Tables\mtab_freqy.tex", style(tex) append cells(b(nostar fmt(a2)) se(fmt(a2) par)) varwidth(50) msign("\$-\$") extracols(8)  mlabels(none) collabels(none) varlabels(me_1090 "Effect of 10th--90th \%ile move in diversity") 

estout col8_me_w using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Results\Tables\mtab_freqy.tex", style(tex) append cells(b(nostar fmt(a2)) se(fmt(a2) par)) varwidth(50) msign("\$-\$") extracols(-5/0 2/7) mlabels(none) collabels(none) varlabels(me_1090 "Effect of 10th--90th \%ile move in within-group") 

estout col8_me_b using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Results\Tables\mtab_freqy.tex", style(tex) append cells(b(nostar fmt(a2)) se(fmt(a2) par)) varwidth(50) msign("\$-\$") extracols(-5/0 2/7) mlabels(none) collabels(none) varlabels(me_1090 "Effect of 10th--90th \%ile move in between-group") postfoot("\midrule") 

estout col11_fs col12_fs using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Results\Tables\mtab_freqy.tex", style(tex) append cells(b(nostar fmt(a2)) se(fmt(a2) par)) keep(mdist_addis, relax) stats(fs_fsta, fmt(a2) labels("First-stage \$F\$ statistic") layout(@)) varwidth(50) msign("\$-\$") extracols(-9/0) label mlabels(none) collabels(none) posthead("FIRST STAGE&\multicolumn{10}{c}{}&\multicolumn{2}{c}{Population diversity}\\"  "&\multicolumn{10}{c}{}&\multicolumn{2}{c}{(ancestry adjusted)}\\"  "\cmidrule{12-13}") varlabels(,elist(mdist_addis \addlinespace)) postfoot("\bottomrule\addlinespace" "\end{tabular*}") 
