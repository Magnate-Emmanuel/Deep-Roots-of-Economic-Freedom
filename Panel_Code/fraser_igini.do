...
/******************************************************************************************************************/
/* TABLE: Population Diversity and Economic Freedom  in Repeated Cross-Country Data - Robustness to Accounting for Ethnic and Spatial Inequality */
/******************************************************************************************************************/
drop if year < 1970
# delimit 
clear all 
...
/* COLUMN 1 */
/* -------- */
eststo col1: reg $ylist $xlist ${ethineq} $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* COLUMN 2 */
/* -------- */
eststo col2: reg $ylist $xlist $geor ${ethineq} $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
pcorr $ylist $xlist $geor ${ethineq} if e(sample) == 1 
...
/* COLUMN 3 */
/* -------- */
eststo col3: reg $ylist $xlist $geor $climate $ethineq $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
*outreg2 using econfree.rtf, append keep(avg_pdiv_aa $geor) ctitle(OLS) 
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor ${ethineq} $climate if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 4 */
/* -------- */
eststo col4: reg $ylist $xlist $geor $climate $ethineq $ethfrac $ethpolr $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor ${ethineq} $climate $ethfrac $ethpolr if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 5 */
/* -------- */
eststo col5: reg $ylist $xlist $geor $climate $ethineq $ethfrac $ethpolr $ypercap $popsize $oilprod $timedum if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $climate ${ethineq} $ethfrac $ethpolr $ypercap $popsize $oilprod if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $climate ${ethineq} $ethfrac $ethpolr $ypercap $popsize $oilprod $inst if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor ${ethineq} $climate  $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
eststo col11_fs: reg $xlist $ivlist $geor ${ethineq} $climate if smpl_flag1 == 1, vce(cluster cid) noomitted vsquish
...
/* Obtain the first-stage regression coefficients and the first-stage F-statistic */
eststo col12_fs: reg $xlist $ivlist $geor $climate ${ethineq} $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall if smpl_flag2 == 1, vce(cluster cid) noomitted vsquish
...
estout col1 col2 col3 col4 col5 col6  col9  col11 col12 using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Codes\Fraser Institute\PANEL\Results\mtab_panel_egini.tex", style(tex) replace cells(b(star fmt(a2)) se(fmt(a2) par))  keep($xlist $ethineq, relax)  indicate( "Continent dummies=${contall}" "Controls for geography=$geor"  "Controls for climate=$climate" "Controls of ethnic diversity=$ethfrac $ethpolr" "Controls for oil, population, and income=$ypercap $popsize $oilprod" "Controls for Institutions=$inst"  "Colonial History=$colhist" "Legal origin dummies=${legalor}" , labels("\$\times\$" " ")) stats(N pc_pdiv pc_pdiv_w pc_pdiv_b adjr2, fmt(%9.0f a2 a2 a2 a2) labels("Observations"  "Partial \$R^2\$ of population diversity"  "Partial \$R^2\$ of within-group"  "Partial \$R^2\$ of between-group"  "Adjusted \$R^2\$") layout(@ @ @ @ @)) varwidth(50) msign("\$-\$") label prehead("\begin{tabular*}{600pt}{@{\extracolsep{\fill}}lcccccccccccc}" "\cmidrule{1-11}" "Cross-country sample:") numbers mlabels("OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "OLS"  "2SLS" "2SLS") collabels(none) posthead("\cmidrule{1-11}"  "&\multicolumn{10}{c}{Economic Freedom per year, 1970--2017}\\"  "\cmidrule{2-11}") varlabels(, elist(${inst} \addlinespace)) prefoot("\midrule") postfoot("\addlinespace") 

estout col1_me col2_me col3_me col4_me col5_me col6_me  col9_me  col11_me col12_me using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Codes\Fraser Institute\PANEL\Results\mtab_panel_egini.tex", style(tex) append cells(b(nostar fmt(a2)) se(fmt(a2) par)) varwidth(50) msign("\$-\$") extracols(8)  mlabels(none) collabels(none) varlabels(me_1090 "Effect of 10th--90th \%ile move in diversity") 


estout col11_fs col12_fs using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Codes\Fraser Institute\PANEL\Results\mtab_panel_egini.tex", style(tex) append cells(b(nostar fmt(a2)) se(fmt(a2) par)) keep(mdist_addis, relax) stats(fs_fsta, fmt(a2) labels("First-stage \$F\$ statistic") layout(@)) varwidth(50) msign("\$-\$") extracols(-9/0) label mlabels(none) collabels(none) posthead("FIRST STAGE&\multicolumn{8}{c}{}&\multicolumn{2}{c}{Population diversity}\\"  "&\multicolumn{8}{c}{}&\multicolumn{2}{c}{(ancestry adjusted)}\\"  "\cmidrule{10-11}") varlabels(,elist(mdist_addis \addlinespace)) postfoot("\cmidrule{1-11}\addlinespace" "\end{tabular*}") 
