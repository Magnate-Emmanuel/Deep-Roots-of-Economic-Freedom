...
/******************************************************************************************************************/
/* TABLE: Population Diversity and Economic Freedom across Countries - Accounting for Corruption Covariates */
/******************************************************************************************************************/
...
global corrupt "avg_v2exbribe avg_v2excrptps avg_v2lgcrrpt avg_v2jucorrdc avg_v2x_corr"

/******************************************************/
/* Generate the sample indicators for the regressions */
/******************************************************/

qui egen smpl_flag1 = rowmiss($ylist $xlist ${geog} $corrupt $climate ${ethfrac} ${ethpolr}) 
qui replace smpl_flag1 = (smpl_flag1 == 0) 

qui egen smpl_flag2 = rowmiss($ylist $xlist $corrupt ${geog} $climate ${ethfrac} ${ethpolr} ${colhist} ${legalor} ${inst} ${oilprod} ${popsize} ${ypercap}) 
...
/* COLUMN 2 */
/* -------- */
eststo col2: reg $ylist $xlist $corrupt $geor if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
pcorr $ylist $xlist $corrupt $geor if e(sample) == 1 
...
/* COLUMN 3 */
/* -------- */
eststo col3: reg $ylist $xlist $corrupt $geor $climate if smpl_flag1 == 1, vce(robust) noomitted vsquish
*outreg2 using econfree.rtf, append keep(avg_pdiv_aa $geor) ctitle(OLS) 
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $corrupt $geor $climate if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 4 */
/* -------- */
eststo col4: reg $ylist $xlist $corrupt $geor $climate $ethpolr $ethfrac if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $corrupt $climate $ethpolr $ethfrac if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 5 */
/* -------- */
eststo col5: reg $ylist $xlist $corrupt $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $corrupt $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 6 */
/* -------- */
eststo col6: reg $ylist $xlist $corrupt $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $corrupt $geor $climate $linfrac $ethpolr $ypercap $popsize $oilprod $inst if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 7 */
/* -------- */
eststo col7: reg $ylist $xlist $corrupt $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $corrupt $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 8 */
/* --------- */
eststo col8: ivregress 2sls $ylist $corrupt $geor $climate $ethfrac $ethpolr ($xlist = $ivlist) if smpl_flag1 == 1, vce(robust) first noomitted vsquish 
...
/* COLUMN 9 */
/* --------- */
eststo col9: ivregress 2sls $ylist $corrupt $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall ($xlist = $ivlist) if smpl_flag2 == 1, vce(robust) first noomitted vsquish 
