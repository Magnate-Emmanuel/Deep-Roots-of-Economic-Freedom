...
/**************************************************************************************************************************************************************************/
/* TABLE: Population Diversity and the Frequency of Civil Conflict Onset across Countries - Robustness to Accounting for Deep-Rooted Determinants of Economic Development */
/**************************************************************************************************************************************************************************/
...
qui egen smpl_flag1 = rowmiss($ylist $xlist ${geog} $climate ${ethfrac} ${ethpolr} $deepdet) 
qui replace smpl_flag1 = (smpl_flag1 == 0) 

qui egen smpl_flag2 = rowmiss($ylist $xlist ${geog} $climate ${ethfrac} ${ethpolr} ${colhist} ${legalor} ${inst} ${oilprod} ${popsize} $deepdet ${ypercap}) 
...
/* COLUMN 2 */
/* -------- */
eststo col2: reg $ylist $xlist $deepdet $geor if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
pcorr $ylist $xlist $deepdet $geor if e(sample) == 1 
...
/* COLUMN 3 */
/* -------- */
eststo col3: reg $ylist $xlist $deepdet $geor $climate if smpl_flag1 == 1, vce(robust) noomitted vsquish
*outreg2 using econfree.rtf, append keep(avg_pdiv_aa $geor) ctitle(OLS) 
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $deepdet $geor $climate if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 4 */
/* -------- */
eststo col4: reg $ylist $xlist $deepdet $geor $climate $ethfrac $ethpolr if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $deepdet $climate $ethfrac $ethpolr if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 5 */
/* -------- */
eststo col5: reg $ylist $xlist $deepdet $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $deepdet $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 6 */
/* -------- */
eststo col6: reg $ylist $xlist $deepdet $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $deepdet $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 7 */
/* -------- */
eststo col7: reg $ylist $xlist $deepdet $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
/* Obtain the partial R2 of diversity */
pcorr $ylist $xlist $geor $deepdet $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall if e(sample) == 1 
matrix pc_mtrx = r(p_corr) 
...
/* COLUMN 8 */
/* --------- */
eststo col8: ivregress 2sls $ylist $deepdet $geor $climate ($xlist = $ivlist) if smpl_flag1 == 1, vce(robust) first noomitted vsquish 
...
/* COLUMN 9 */
/* --------- */
eststo col9: ivregress 2sls $ylist $deepdet $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall ($xlist = $ivlist) if smpl_flag2 == 1, vce(robust) first noomitted vsquish 
...
estout col1 col2 col3 col4 col5 col6 col7 col8 col9 using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Codes\Fraser Institute\CROSS\Results\mtab_roots.tex", style(tex) replace  cells(b(star fmt(a2)) se(fmt(a2) par)) keep($xlist ${deepdet}, relax)  order($xlist ${deepdet})  indicate("Continent dummies=${contall}"     "Controls for geography=${geor}" "Controls for climate=${climate}"  "Controls for ethnic diversity=${ethfrac} ${ethpolr}"   "Controls for institutions= ${inst}" "Controls for Colonial History and Legal Origins=${colhist} ${legalor}"  "Controls for oil, population, and income=${oilprod} ${popsize} ${ypercap}", labels("\$\times\$" " ")) stats(N pc_pdiv adjr2, fmt(%9.0f a2 a2)  labels("Observations"   "Partial \$R^2\$ of population diversity"  "Adjusted \$R^2\$") layout(@ @ @))   varwidth(46) msign("\$-\$") label   prehead("\begin{tabular*}{675pt}{@{\extracolsep{\fill}}lccccccccc}"           "\toprule"           "Cross-country sample:&\multicolumn{5}{c}{Global}&\multicolumn{2}{c}{Old World}&\multicolumn{2}{c}{Global}\\"           "\cmidrule(r){2-6}\cmidrule(lr){7-8}\cmidrule(l){9-10}")   numbers mlabels("OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "2SLS" "2SLS") collabels(none)   posthead("\midrule"            "&\multicolumn{9}{c}{Log number of new PRIO25 civil conflict onsets per year, 1960--2017}\\"            "\cmidrule{2-10}")   varlabels(, elist(ln_frontdist1500 \addlinespace))   prefoot("\midrule") postfoot("\addlinespace") 

estout col1_me col2_me col3_me col4_me col5_me col6_me col7_me col8_me col9_me using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Codes\Fraser Institute\CROSS\Results\mtab_roots.tex", style(tex) append   cells(b(nostar fmt(a2)) se(fmt(a2) par))   varwidth(46) msign("\$-\$")   mlabels(none) collabels(none)   varlabels(me_1090 "Effect of 10th--90th \%ile move in diversity", elist(me_1090 \addlinespace)) 

estout col1 col2 col3 col4 col5 col6 col7 col8 col9 using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Codes\Fraser Institute\CROSS\Results\mtab_roots.tex", style(tex) append   cells(none)   stats(fs_fsta, fmt(a2) labels("First-stage \$F\$ statistic") layout(@))   varwidth(46) msign("\$-\$")   mlabels(none) collabels(none)   postfoot("\bottomrule\addlinespace" "\end{tabular*}") 
