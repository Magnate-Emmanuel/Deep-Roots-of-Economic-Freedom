...
/*********************************************************************************************************************************************************/
/* TABLE: Population Diversity and the Frequency of Civil Conflict Onset across Countries - The Analysis under Initial Values of Time-Varying Covariates */
/*********************************************************************************************************************************************************/
...
label variable xconst_1960_1999_fnm_p4_v17 "Executive constraints in initial year" 
label variable democ_1960_1999_fnm_p4_v17  "Democracy score in initial year" 
label variable autoc_1960_1999_fnm_p4_v17  "Autocracy score in initial year" 
...
label variable lnpop_1960_1999_fnm_wdi          "Log population in initial year" 
label variable lngdppc_cu_usd_1960_1999_fnm_wdi "Log GDP per capita in initial year" 
...
global popsize " lnpop_1960_1999_fnm_wdi " 
global ypercap " lngdppc_cu_usd_1960_1999_fnm_wdi"  
global contall " avg_africa avg_asia avg_namerica avg_samerica avg_oceania avg_europe " 
global inst "democ_1960_1999_fnm_p4_v17 autoc_1960_1999_fnm_p4_v17 xconst_1960_1999_fnm_p4_v17"

/******************************************************/
/* Generate the sample indicators for the regressions */
/******************************************************/

qui egen smpl_flag1 = rowmiss($ylist $xlist $geor $ethfrac $ethpolr) 
qui replace smpl_flag1 = (smpl_flag1 == 0) 

qui egen smpl_flag2 = rowmiss($ylist $xlist $geor $ethfrac $ethpolr $ypercap $popsize $inst $legalor $colhist $contall $oilprod) 
...
/* COLUMN 7 */
/* -------- */
eststo col7: reg $ylist $xlist $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall if smpl_flag1 == 1, vce(robust) noomitted vsquish
...
/* COLUMN 8 */
/* --------- */
eststo col8: ivregress 2sls $ylist $geor $climate ($xlist = $ivlist) if smpl_flag1 == 1, vce(robust) first noomitted vsquish 
...
/* COLUMN 9 */
/* --------- */
eststo col9: ivregress 2sls $ylist $geor $climate $ethfrac $ethpolr $ypercap $popsize $oilprod $inst $legalor $colhist $contall ($xlist = $ivlist) if smpl_flag2 == 1, vce(robust) first noomitted vsquish 
...
estout col1 col2 col3 col4 col5 col6 col7 col8 col9 using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Codes\Fraser Institute\CROSS\Results\stab_initial.tex", style(tex) replace cells(b(star fmt(a2)) se(fmt(a2) par)) keep($xlist $inst ${popsize} ${ypercap}, relax) order($xlist ${exconst} ${polregs} ${popsize} ${ypercap}) indicate("Continent dummies= ${contall}"  "Controls for geography= ${geor}" "Controls for Climate= ${climate}" "Controls for ethnic diversity= ${ethfrac} ${ethpolr}" "Controls for legal origin and colonial history= ${colhist} ${legalor}"  "Control for oil or gas reserve discovery= ${oilprod}", labels("\$\times\$" " ")) stats(N pc_pdiv adjr2, fmt(%9.0f a2 a2)  labels("Observations"  "Partial \$R^2\$ of population diversity"  "Adjusted \$R^2\$") layout(@ @ @))  varwidth(46) msign("\$-\$") label  prehead("\begin{tabular*}{685pt}{@{\extracolsep{\fill}}lccccccccc}"  "\toprule"  "Cross-country sample:&\multicolumn{5}{c}{Global}&\multicolumn{2}{c}{Old World}&\multicolumn{2}{c}{Global}\\"   "\cmidrule(r){2-6}\cmidrule(lr){7-8}\cmidrule(l){9-10}")  numbers mlabels("OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "OLS" "2SLS" "2SLS") collabels(none)  posthead("\midrule"  "&\multicolumn{9}{c}{Economic Freedom per year, 1970--2017}\\"  "\cmidrule{2-10}")  varlabels(, elist(${ypercap} \addlinespace))  prefoot("\midrule") postfoot("\addlinespace") 

estout col1_me col2_me col3_me col4_me col5_me col6_me col7_me col8_me col9_me using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Codes\Fraser Institute\CROSS\Results\stab_initial.tex", style(tex) append  cells(b(nostar fmt(a2)) se(fmt(a2) par))  varwidth(46) msign("\$-\$")  mlabels(none) collabels(none)   varlabels(me_1090 "Effect of 10th--90th \%ile move in diversity", elist(me_1090 \addlinespace)) 

estout col1 col2 col3 col4 col5 col6 col7 col8 col9 using "C:\Users\sarfo\Desktop\GRADUATE SCHOOL\SCHOOLS\HSE\COURSES\RESEARCH\Economic Freedom Predictors\Codes\Fraser Institute\CROSS\Results\stab_initial.tex", style(tex) append cells(none)  stats(fs_fsta, fmt(a2) labels("First-stage \$F\$ statistic") layout(@))  varwidth(46) msign("\$-\$")  mlabels(none) collabels(none)  postfoot("\bottomrule\addlinespace" "\end{tabular*}") 
